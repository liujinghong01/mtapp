        ...
        <div class="select-main">
          ...
          <div class="main-list" style="height: 100%">
            <scroll  :on-infinite="onInfinite"  style="-webkit-overflow-scrolling: touch;top: 70px;">
              <p v-for=" (item,index) in selectInfo.list " v-show="!(item==='出库凭证号'||item==='送货单号'||item==='采购单号'||item==='生产单号')"  @click="onCheck(item,index)">{{ item }}</p>
              ...
              <div v-if="selectInfo.selectType==='number'&&requestData.query.bill_no===''" slot="infinite" class="text-center">请输入单号</div>
            </scroll>
          </div>
        </div>
      </div>
...
<script>
  import { billSearch } from '@/api/storage/storagePull'
        ...
        onCheck(item,index){
          switch (this.selectInfo.selectType){
            ...
            case 'number':{
              ...
              this.$router.forward('/storage/storagePull/singleDetail/action/'+item+'/'+this.singleType)
              break
            }
          }

  ...
  <div class="mw-page page" v-nav="{hideNavbar:true}">
    ...
    <div :class="showFilter?'filter active':'filter'">
      ...
      <list class="list-ios" v-show="showFilter" style="position: relative">
        <von-input type="text" v-model="query.project_no" placeholder="输入客户定单编号"></von-input>
...
<script>
  ...
  import { referHis,list } from '@/api/customerOrder/order'
    ...
    methods: {
      ...
      queryHistory(){
        ...
        list(reqData).then(res=>{
          this.historys = this.historys.concat(res.cst_order_list)
      ...
      chooseItem(item){
        if(this.chosenItem.project_id===item.project_id){
          return
        }
      ...
      arrowClick(item){
        this.$router.forward('/customerOrder/order/detail/check/'+item.project_id)
      },
      saveData(){
        const reqData = {
          project_id:this.chosenItem.project_id
        }
        referHis(reqData).then(res=>{
          console.log(res)
          this.$router.forward('/customerOrder/order/detail/edit/'+res.project_head.project_id)
        }).catch(error=>{
          $toast.show(error.description)
        })
      },

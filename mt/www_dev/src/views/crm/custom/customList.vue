<template>
  <div class="mw-page page crm" v-nav="{hideNavbar:true}">
    <div class="mw-page-header">
      ...
      <span>我的客户</span>
      <i class="right" @click="$router.forward('/crm/newCustom')" v-if="chosenCompany">新增</i>
    ...
    <div class="mw-page-content has-tab" v-if="chosenCompany">
      <div style="width: 100%;background-color: white;height: 50px">
        <mw-search placeholder="查询客户信息"  v-model="searchVal" :search="onSearch" cancelText="取消" :cancelAction="onCancel"></mw-search>
      </div>
        ...
        <div style="height: 100%;position: relative">
          <scroll class="page-content" :on-infinite="onInfinite" :on-refresh="onRefresh">
            <swipe-item :obj="item"  v-for="item in cstInfos"  v-on:ItemClick="toInfo" swipeItemText="删除" v-on:SwipeItemClick="delItem">
              ...
              <span class="item-text item-text-top">{{item.cst_company_name}}</span><br>
      ...
      <div style="flex: 1" v-else="isShowSearchResult">
        <list v-if="searchList.length">
          <item class="item-icon-right" v-for="item in searchList" @click.native="toInfo(item)">
            <img src="../../../../static/images/company/logo_company.png" class="item-img">
            <span class="item-text item-text-top">{{item.cst_company_name}}</span><br>
            <span class="item-text item-text-bottom">地址:{{item.cst_company_address}}</span>
          </item>
        </list>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      toInfo(item){
        this.$router.forward({path:'/crm/customInfo',query:{id:item.cst_company_id}});
      }
    }
  }
</script>

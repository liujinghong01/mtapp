<template>
    <div class="page has-navbar"
        ...
        <div class="page-content">
          <von-checkbox :options="supplyList" v-model="thisList" theme="positive"></von-checkbox>
          <div class="div-bottom">
            <div class="selected-label" v-for="item in thisList" :key="item.company_id">{{supplyList[item].substr(0,3)}}...</div>
            <md-button
...
<script>
  ...
  export default {
    ...
    created(){
      let supplyList = JSON.parse(window.sessionStorage.getItem('supplyList'));
      for(var i in supplyList){
        this.supplyList[i] = supplyList[i].sup_company_name;
        this.listIds[i] = supplyList[i].sup_company_id;
      }
      //this.supplyList;
    },
    methods:{

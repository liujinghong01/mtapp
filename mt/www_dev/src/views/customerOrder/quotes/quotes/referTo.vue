...
<script>
  ...
  import { list,computeList,headDetail,quoteLines,computeDetail,referHis ,relateCompute } from '@/api/customerOrder/quote'
  ...
  export default{
    ...
    methods:{
      ...
      setList(refresh){
        ...
        computeList(requestData).then(res=>{
          if (res.list.length>0) {
            if(!refresh){
              that.items[1].list= that.items.concat(res.list);
            }else{
              that.items[1].list = res.list;
            }
      ...
      referTo(item){
        ...
        if(this.tabIndex===0){
          ...
          const reqData = {
            compute_ids:[item.compute_id],//	核算单Id	array<number>
            quote_id:this.quote_id
          }
          relateCompute(reqData).then(res => {
            $toast.show('参考成功',600)
            $router.go(-1)
          }).catch(error => {
            $toast.show(error.description)

...
import com.mt.common.utils.HttpUtil;
...
public final class ThirdPartyLoginHelper {
	 ...
	 * @param openid
	 */
	public static final User getQQUserinfo(String token, String openid) throws Exception {
		User user = new User();
		...
		if (json.getIntValue("ret") == 0) {
			...
			if ("女".equals(sex)) {
				user.setGender(Byte.parseByte("2"));
			} else {
				user.setGender(Byte.parseByte("1"));
			}
		} else {
	...
	/** 获取微信用户信息 */
	public static final User getWxUserinfo(String token, String openid) throws Exception {
		User user = new User();
	 ...
	 * @return
	 */
	public static final User getSinaUserinfo(String token, String uid) throws Exception {
		User user = new User();

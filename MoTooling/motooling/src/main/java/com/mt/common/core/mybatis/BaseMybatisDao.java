...
import com.mt.common.core.mybatis.page.MysqlDialect;
...
public class BaseMybatisDao<T> extends CommonDao {
	...
	/**
	 * 根据Sql id 去查询 分页对象, 先取count, 再确定 pageNo的有效性，然后才查询数据. modified by fhk.
	 * @param sqlId			对应mapper.xml 里的Sql Id
	 * @param params		参数<String,Object>
	 * @param pageNo		number
	 * @param pageSize		size
	 * @return
	 */
	public Pagination findByPageBySqlIdAndCheckPageNoValidity(String sqlId,
										Map<String, Object> params, Integer pageNo, Integer pageSize) {
		...
		try {
			...
			Integer totalPage = (new Double(Math.ceil(page.getTotalCount() / page.getPageSize()))).intValue();
			if(pageNo > totalPage) page.setPageNo(totalPage);
